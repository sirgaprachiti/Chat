...
async function sendViaSendGrid(to, rawToken) {
  ...
  const msg = {
    ...
    subject: 'Password reset',
    text: `Reset your password: ${resetUrl}`,
    html: `<p>Click to reset: <a href="${resetUrl}">Reset password</a></p><pre>${resetUrl}</pre>`
  };
  const res = await sgMail.send(msg);
  return res;
}
...
module.exports = async function sendResetEmail(email, rawToken) {
  try {
    if (process.env.SENDGRID_API_KEY) {
      const r = await sendViaSendGrid(email, rawToken);
      console.log('sendResetEmail SendGrid result:', Array.isArray(r) ? r[0] : r);
      return r;
    } else {
      const r = await sendViaNodemailer(email, rawToken);
      console.log('sendResetEmail Nodemailer result:', r);
      return r;
    }
  } catch (err) {
    console.error('sendResetEmail error:', err && err.stack ? err.stack : err);
