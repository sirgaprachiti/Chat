...
async function sendViaSendGrid(to, token) {
  ...
  const msg = {
    ...
    subject: 'Please verify your email',
    text: `Verify your email: ${verifyUrl}`,
    html: `<p>Please verify your email by clicking <a href="${verifyUrl}">this link</a>.</p><pre>${verifyUrl}</pre>`
  };
  const res = await sgMail.send(msg);
  return res;
}
...
async function sendViaNodemailer(to, token) {
  // dev fallback only: use Ethereal if no SMTP env configured
  let transporter;
  if (process.env.SMTP_HOST && process.env.SMTP_USER) {
    transporter = nodemailer.createTransport({
      host: process.env.SMTP_HOST,
  ...
  return { nodemailerInfo: info, preview: nodemailer.getTestMessageUrl ? nodemailer.getTestMessageUrl(info) : undefined };
}

module.exports = async function sendVerificationEmail(email, token) {
  try {
    if (process.env.SENDGRID_API_KEY) {
      const r = await sendViaSendGrid(email, token);
      console.log('sendVerificationEmail SendGrid result:', Array.isArray(r) ? r[0] : r);
      return r;
    } else {
      const r = await sendViaNodemailer(email, token);
      console.log('sendVerificationEmail Nodemailer result:', r);
      return r;
    }
  } catch (err) {
    console.error('sendVerificationEmail error:', err && err.stack ? err.stack : err);
